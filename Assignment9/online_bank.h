...
// from-WinHtut-to-lonelone-100
    ...
    char from[14]={'T', 'r', 'a', 'n', 's', 'f', 'e', 'r', '_','F','r','o','m','_'};
    char to[4]={'_','t','o','_'};
    char beforeAmount[2] = {'_', '$'};
    ...
    if(who == 't'){
        ...
        for(int i = 0; i<2; i++){
            db[transfer].trc[space_array[transfer]-15].note[index_point]=beforeAmount[i];
            index_point++;
        }
        ...
        strftime(recordTime, 30, "_at_%d-%b-%Y_%I:%M:%S%p", local_time);

        int datetime_str_count = charCounting(recordTime);

        for(int i = 0; i<datetime_str_count; i++){
            db[transfer].trc[space_array[transfer]-15].note[index_point]=recordTime[i];
            index_point++;
        }
        ...
        for(int i = 0; i<2; i++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=beforeAmount[i];
            index_point++;
        }
        ...
        for(int i = 0; i<datetime_str_count; i++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=recordTime[i];
            index_point++;
        }
